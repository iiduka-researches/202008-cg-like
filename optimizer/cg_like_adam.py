...
class CGLikeAdam(Optimizer):
    ...
    def __init__(self, params, alpha_type=required, beta_type=required, gamma_type=required,
                 delta=.9, zeta=.999, eps=1e-8, weight_decay=0, amsgrad=False):
        if not 0.0 <= eps:
            raise ValueError("Invalid epsilon value: {}".format(eps))
        if not 0.0 <= weight_decay:
            raise ValueError("Invalid weight_decay value: {}".format(weight_decay))
        defaults = dict(alpha_type=alpha_type, beta_type=beta_type, gamma_type=gamma_type,
                        delta=delta, zeta=zeta, eps=eps, weight_decay=weight_decay, amsgrad=amsgrad)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                zeta = group['zeta']
                exp_avg_sq.mul_(zeta).addcmul_(grad, grad, value=1 - zeta)
                if amsgrad:
                    ...
                    bias_correction2 = 1 - zeta ** state['step']
